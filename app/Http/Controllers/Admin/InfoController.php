...
class InfoController extends Controller
{
	...
	public function userInfo()
	{
		...
		$list = $InfoUser->where('status',1)->order('created_at','desc')->paginate(15);
	...
	public function infoList(Request $request)
	{
		...
		$result = Info::where($where)->order('created_at','desc')->paginate(15);
	...
	{
		$Model = new InfoCommon();
		$brand = $Model->where('type',1)->order('created_at','desc')->paginate(10,['*'],'bpage');
		$brandCount = $Model->where('type',1)->count();
		$model = $Model->where('type',2)->order('created_at','desc')->paginate(10,['*'],'mpage');
		$modelCount = $Model->where('type',2)->count();
		$country = $Model->where('type',3)->order('created_at','desc')->paginate(10,['*'],'cpage');
		$countryCount = $Model->where('type',3)->count();
		$os = $Model->where('type',4)->order('created_at','desc')->paginate(10,['*'],'lpage');
		$osCount = $Model->where('type',4)->count();
		$type = $Model->where('type',5)->order('created_at','desc')->paginate(10,['*'],'tpage');
		$typeCount = $Model->where('type',5)->count();
		$tag = $Model->where('type',6)->order('created_at','desc')->paginate(10,['*'],'gpage');
