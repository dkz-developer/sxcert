...
class InfoController extends Controller
{
	...
	{
		$Model = new InfoCommon();
		$brand = $Model->where('type',1)->orderBy('id', 'desc')->paginate(10);
		$brandCount = $Model->where('type',1)->count();
		$model = $Model->where('type',2)->orderBy('id', 'desc')->paginate(10);
		$modelCount = $Model->where('type',2)->count();
		$country = $Model->where('type',3)->orderBy('id', 'desc')->paginate(10);
		$countryCount = $Model->where('type',3)->count();
		$os = $Model->where('type',4)->orderBy('id', 'desc')->paginate(10);
		$osCount = $Model->where('type',4)->count();
		$type = $Model->where('type',5)->orderBy('id', 'desc')->paginate(10);
		$typeCount = $Model->where('type',5)->count();
		$tag = $Model->where('type',6)->orderBy('id', 'desc')->paginate(10);
